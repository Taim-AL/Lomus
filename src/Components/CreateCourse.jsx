...
const CreateCourse = () => {
    ...
    const [courseInfo, setCourseInfo] = useState({title:"" ,photo:postPhoto,cost:"",description:"",requirements:"",expectations:"",for_child:"",tags:tags});
    ...
    const handelCreateCousre = async (event)=> {
        ...
        setloading(true);
        try{
        await AxiosAuth.post("/tch/courses",courseInfo,{headers:{"Content-Type" : 'multipart/form-data'}}).then(response =>{
            console.log(response.data);
            setloading(false); 
            setMsg("The Course has been created");
            setCourseInfo({});
            navigate(-1)
        })
        }catch(err){
            ...
            <div className="container-outer-create-course ">
                ...
                <div className="app1">
                    <div className="parent1">
                        <div className="file-upload1">
                            ...
                            <input type="file" required  onChange={(e) => handelSetTheImage(e)}/>
                        </div>
                    </div>
                </div>
                ...
                <Row className="mx-0">
                    {inputsDataCreateCourse.map((e,i)=>{
                        return(
                            <Col md='6' xs='12' key={i} style={{marginTop:"0.5rem"}}>
                                ...
                                <input type="text" required={e.isRequired} onChange={event => setCourseInfo(prev=>{return({...prev , [e.proberty]:event.target.value})})}  className="input-create-course"/>
                            </Col>
                        )
                    })}
                    ...
                    <Col md='12'>
                        ...
                        <textarea value={courseInfo?.description} required onChange={e => setCourseInfo(prev=>{return({...prev , description:e.target.value})})} className="input-create-course"> </textarea>
                    </Col>
                    ...
                    [...Array(5)].map((e,index)=>{
                        return(
                            <Col md='4' key={index}>
                                ...
                                <input required={req}  onChange={e=> handelAddTag(e.target.value , [index])} type="text" className="input-create-course" />
                            </Col>
                        )
                    })
                    }
                    <Col md='6'>
                            ...
                            >
                                <div style={{display:"flex"}}>
                                    <FormControlLabel required value="1" onChange={e => setCourseInfo(prev=>{return({...prev , for_child:e.target.value})})} control={<Radio />} label="Yes" />
                                    <FormControlLabel required value="0" onChange={e => setCourseInfo(prev=>{return({...prev , for_child:e.target.value})})} control={<Radio />} label="No" />
                                </div>
                        </RadioGroup>
                    </Col>
